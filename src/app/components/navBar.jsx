import Link from 'next/link';
import React from 'react';
import LogoSvg from '../../../public/logo.svg';
import Image from 'next/image';
const NavBar = () => {
  const overlayStyle = {
    position: 'fixed',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    zIndex: 1000,
    background: 'rgba(0, 0, 0, 0.3)',
    opacity: 0,
    MozTransform: 'translate3d(100%, 0, 0)',
    msTransform: 'translate3d(100%, 0, 0)',
    OTransform: 'translate3d(100%, 0, 0)',
    WebkitTransform: 'translate3d(100%, 0, 0)',
    transform: 'translate3d(100%, 0, 0)',
    transition: 'opacity 100ms, transform 0s 100ms',
  };
  const sidebarStyle = {
    position: 'fixed',
    top: 0,
    right: 0,
    zIndex: 1100,
    width: '300px',
    height: '100%',
    MozTransform: 'translate3d(100%, 0, 0)',
    msTransform: 'translate3d(100%, 0, 0)',
    OTransform: 'translate3d(100%, 0, 0)',
    WebkitTransform: 'translate3d(100%, 0, 0)',
    transform: 'translate3d(100%, 0, 0)',
    transition: 'all 200ms',
  };
  const contentStyle = {
    height: '100%',
    overflow: 'auto',
    padding: '48px 0 0',
    background: '#fff',
  };
  const iconStyle = {
    position: 'absolute',
    width: '24px',
    height: '24px',
    right: '8px',
    top: '8px',
  };
  const positionedStyle = {
    position: 'absolute',
    top: '6px',
    right: '14px',
  };

  return (
    <>
      <header
        style={{ top: '0' }}
        className="bg-white fixed w-full z-50"
        id="header_container"
      >
        <div className="h-18 flex flex-row items-center px-4 py-4 md:px-10">
          <Link href={'/'}>
            <svg width="56" height="36" viewBox="0 0 56 36" fill="none">
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M53.0734 21.0774C52.9889 21.8371 52.8201 22.4281 52.6513 22.9346C52.398 23.4412 52.2291 23.8633 51.8914 24.201C51.6382 24.4542 51.3005 24.7075 50.9628 24.9608C50.5407 25.214 50.1186 25.4673 49.5276 25.6361L47.6703 26.2271V28.1688V30.3638L42.3517 27.2402L41.5919 26.8181L40.7477 26.9025C39.9879 26.9869 39.1437 27.0713 38.2995 27.0713C36.0201 27.1558 30.8703 27.2402 28 27.2402C25.1296 27.2402 19.8954 27.1558 17.6161 27.0713C15.5055 26.9025 13.5638 26.7336 11.8753 26.5648C9.68037 26.3115 8.07636 26.0582 6.81002 25.7206C6.13464 25.5517 5.54369 25.2985 5.12158 25.0452C4.69946 24.7919 4.36178 24.5387 4.10851 24.201C3.77082 23.8633 3.51755 23.4412 3.34871 22.9346C3.17987 22.4281 3.01103 21.8371 2.9266 21.0774C2.67334 19.4733 2.67334 17.1095 2.67334 14.999C2.67334 12.8884 2.67334 10.4402 2.9266 8.92057C3.01103 8.16077 3.17987 7.56981 3.34871 7.06329C3.60198 6.55676 3.77082 6.13465 4.10851 5.71253C4.36178 5.37484 4.69946 5.12158 5.12158 4.86832C5.54369 4.61504 6.13464 4.36179 6.89444 4.19293C8.16078 3.85525 9.7648 3.60198 11.9598 3.34872C13.6482 3.09545 15.5899 2.92661 17.7005 2.84218C19.9799 2.75776 25.214 2.67334 28.0844 2.67334C30.9548 2.67334 36.1045 2.75776 38.3839 2.92661C40.4944 3.09545 42.4362 3.26429 44.1246 3.43313C46.3196 3.6864 47.9236 3.93968 49.1899 4.27735C49.8653 4.44621 50.4563 4.69947 50.8784 4.95273C51.3005 5.206 51.6382 5.45926 51.8914 5.79695C52.2291 6.13465 52.4824 6.55676 52.6513 7.06329C52.8201 7.56981 52.9889 8.16077 53.0734 8.92057C53.3266 10.4402 53.3266 12.8884 53.3266 14.999C53.3266 17.1095 53.3266 19.4733 53.0734 21.0774H53.0734Z"
                fill="white"
              ></path>
              <path
                d="M9.98901 8.08113H9.98226C9.17377 8.07283 8.51049 8.71951 8.49854 9.52798C8.51204 10.3304 9.17362 10.9701 9.97605 10.9565L9.98226 10.9564C10.776 10.972 11.4321 10.3411 11.4475 9.54741C11.4477 9.54094 11.4478 9.53446 11.4478 9.52798C11.4446 8.72556 10.7914 8.07775 9.98901 8.08113H9.98901Z"
                fill="#FF4338"
              ></path>
              <path
                d="M46.1865 8.08113C45.3841 8.0741 44.728 8.71896 44.7209 9.52124V9.52798C44.7257 10.3218 45.3731 10.9615 46.167 10.9567C46.1734 10.9567 46.1799 10.9565 46.1865 10.9564C46.989 10.9733 47.6532 10.3366 47.6701 9.53418L47.6702 9.52798C47.6581 8.71951 46.995 8.07283 46.1865 8.08113H46.1865Z"
                fill="#FF4338"
              ></path>
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M24.3911 11.7125C23.0892 11.6972 21.8006 11.9762 20.6217 12.5287V14.7279H21.3946C22.1532 14.2231 23.0463 13.958 23.9576 13.9674C25.2767 13.9674 25.7668 14.4311 25.7668 15.3214V15.9601C25.1197 15.8009 24.4546 15.7261 23.7882 15.7373C21.508 15.7373 19.8494 16.7391 19.8494 18.7796C19.8494 20.7087 21.2069 21.7105 23.0533 21.7105C24.2594 21.7105 25.2019 21.4134 25.9181 20.6345C26.1065 21.4134 26.5021 21.5989 27.3691 21.5989H28.5188V14.8481C28.5187 12.6029 27.1239 11.7125 24.3911 11.7125H24.3911ZM25.7668 19.2623C25.3144 19.6704 24.7491 20.0043 24.0706 20.0043C23.1472 20.0043 22.5253 19.5777 22.5253 18.6316C22.5253 17.6855 23.2413 17.2959 24.3533 17.2959C24.8268 17.2931 25.2997 17.3304 25.7668 17.4074L25.7668 19.2623Z"
                fill="#FF4338"
              ></path>
              <path
                d="M40.319 15.682L42.7163 13.084C43.0236 12.7509 43.1942 12.3143 43.1942 11.861H40.9679L37.4993 15.7748V10.1363C37.4993 8.83808 37.3322 8.61548 36.442 8.61548H34.791V21.5998H37.4993V18.5389L38.5008 17.5001L41.0793 21.5993H43.5836C43.5834 21.1146 43.4462 20.6399 43.1876 20.2299L40.319 15.682Z"
                fill="#FF4338"
              ></path>
              <path
                d="M44.8491 13.5921V21.5992H47.5575V11.8608H46.5805C45.6244 11.8608 44.8493 12.636 44.8491 13.5921V13.5921Z"
                fill="#FF4338"
              ></path>
              <path
                d="M9.58521 11.8608H8.60815V21.5992H11.3164V13.5921C11.3164 12.636 10.5413 11.8608 9.58521 11.8608V11.8608Z"
                fill="#FF4338"
              ></path>
              <path
                d="M16.597 18.6684V13.6789H18.5633V11.861H16.597V9.98526C16.597 9.58707 16.2744 9.26416 15.876 9.26416H15.8758H13.8887V11.861H12.7388V13.6789H13.8887V18.9838C13.8887 21.0057 15.0204 21.7478 16.8936 21.7478C17.4727 21.7533 18.0489 21.6655 18.6001 21.4878V19.8559H17.8583C16.968 19.8558 16.597 19.596 16.597 18.6684V18.6684Z"
                fill="#FF4338"
              ></path>
              <path
                d="M31.9125 8.61499H30.2615V21.5992H32.9698V10.1363C32.9698 8.83743 32.8213 8.61499 31.9125 8.61499L31.9125 8.61499Z"
                fill="#FF4338"
              ></path>
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M55.7221 8.5528C55.6044 7.55325 55.3754 6.78176 55.1876 6.21811C54.9076 5.37825 54.4782 4.64617 53.9099 4.0389C53.3711 3.3766 52.7472 2.95646 52.2538 2.66042C51.5628 2.24576 50.7353 1.91019 49.8594 1.68914C48.5274 1.33541 46.9018 1.06246 44.4311 0.777382L44.4108 0.77513L44.3907 0.773018C42.7825 0.612202 40.8873 0.445046 38.5971 0.261839L38.5892 0.261144L38.5813 0.260578C36.1816 0.0828653 30.8991 0 28.0844 0C25.1745 0 19.8841 0.0860989 17.6016 0.170674L17.5975 0.170809L17.5937 0.170953C15.3772 0.259596 13.3639 0.437029 11.6085 0.698173C9.18315 0.979021 7.57591 1.2486 6.25807 1.596C5.3176 1.81043 4.47282 2.13996 3.74623 2.576C3.22366 2.88948 2.55504 3.34199 1.99617 4.07392C1.5542 4.63393 1.29644 5.16832 1.08862 5.59916C1.04684 5.68597 1.00392 5.77517 0.957624 5.86762L0.87264 6.03773L0.812413 6.21811C0.624578 6.78176 0.395654 7.55325 0.277886 8.5528C0 10.2882 0 12.7891 0 14.999C0 17.1705 0 19.6286 0.276618 21.4345C0.393968 22.4391 0.624014 23.2142 0.812413 23.7799C1.09242 24.6197 1.52184 25.3518 2.09016 25.9591C2.6289 26.6214 3.25278 27.0415 3.74622 27.3376C4.43721 27.7522 5.26471 28.0878 6.14058 28.3089C7.47262 28.6626 9.09817 28.9355 11.5689 29.2206L11.5892 29.2229L11.6093 29.225C13.2176 29.3858 15.1127 29.553 17.4029 29.7362L17.4599 29.7408L17.5172 29.7429C19.7997 29.8274 25.0901 29.9136 28 29.9136C30.9023 29.9136 36.0033 29.8301 38.351 29.7446C39.3183 29.7415 40.2368 29.649 41.0254 29.5615L41.0256 29.5617L46.3165 32.669L50.3437 35.0342V30.3638V28.1828C51.2307 27.9177 51.8688 27.5348 52.3383 27.2531L52.4565 27.1822L52.567 27.0995C52.9548 26.8087 53.3943 26.4788 53.7818 26.0912C54.3797 25.4935 54.686 24.8625 54.9321 24.3553C54.9674 24.2826 55.0037 24.2076 55.0424 24.1304L55.1274 23.9603L55.1876 23.7799C55.376 23.2142 55.606 22.4391 55.7234 21.4345C56 19.6286 56 17.1705 56 14.999C56 12.7891 56 10.2882 55.7221 8.5528V8.5528ZM53.0734 21.0774C52.989 21.8372 52.8202 22.4282 52.6513 22.9347C52.398 23.4413 52.2292 23.8634 51.8915 24.201C51.6383 24.4543 51.3005 24.7076 50.9629 24.9608C50.5408 25.2141 50.1187 25.4674 49.5277 25.6362L47.6704 26.2272V28.1689V30.3639L42.3518 27.2402L41.592 26.8181L40.7478 26.9026C39.988 26.987 39.1438 27.0714 38.2996 27.0714C36.0202 27.1558 30.8704 27.2402 28.0001 27.2402C25.1297 27.2402 19.8955 27.1558 17.6161 27.0714C15.5056 26.9026 13.5639 26.7337 11.8754 26.5649C9.68045 26.3116 8.07644 26.0583 6.8101 25.7207C6.13472 25.5518 5.54377 25.2986 5.12166 25.0453C4.69955 24.792 4.36186 24.5388 4.10859 24.201C3.7709 23.8634 3.51763 23.4413 3.34879 22.9347C3.17995 22.4282 3.01111 21.8372 2.92668 21.0774C2.67342 19.4734 2.67342 17.1096 2.67342 14.9991C2.67342 12.8885 2.67342 10.4402 2.92668 8.92066C3.01111 8.16085 3.17995 7.56989 3.34879 7.06337C3.60206 6.55684 3.7709 6.13473 4.10859 5.71261C4.36186 5.37492 4.69955 5.12166 5.12166 4.8684C5.54377 4.61512 6.13472 4.36187 6.89452 4.19301C8.16086 3.85533 9.76488 3.60206 11.9598 3.3488C13.6483 3.09554 15.59 2.92669 17.7006 2.84226C19.98 2.75784 25.2141 2.67342 28.0845 2.67342C30.9548 2.67342 36.1046 2.75784 38.384 2.92669C40.4945 3.09553 42.4362 3.26437 44.1247 3.43321C46.3197 3.68648 47.9236 3.93976 49.19 4.27743C49.8654 4.44629 50.4563 4.69955 50.8785 4.95282C51.3005 5.20608 51.6383 5.45934 51.8915 5.79703C52.2292 6.13473 52.4825 6.55684 52.6513 7.06337C52.8202 7.56989 52.989 8.16085 53.0734 8.92066C53.3267 10.4402 53.3267 12.8885 53.3267 14.9991C53.3267 17.1096 53.3267 19.4734 53.0734 21.0774L53.0734 21.0774Z"
                fill="#FF4338"
              ></path>
            </svg>{' '}
          </Link>
          <div className="flex flex-1 justify-end">
            <div className="language-currency-select cursor-pointer hidden p-2 mr-2 lg:inline-block">
              <div className="flex text-gray2 hover:text-gray1">
                <span>
                  English
                  <span> · </span>
                  USD $
                </span>
                <svg
                  height="24"
                  viewBox="0 0 24 24"
                  width="24"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="#313140"
                  class="fill-current"
                >
                  <path
                    clip-rule="evenodd"
                    d="M7.97 9.97a.75.75 0 011.06 0L12 12.94l2.97-2.97a.75.75 0 111.06 1.06l-3.5 3.5a.75.75 0 01-1.06 0l-3.5-3.5a.75.75 0 010-1.06z"
                    fill-rule="evenodd"
                  ></path>
                </svg>
              </div>
            </div>
            <a className="hidden p-2 mr-2 lg:inline-block" href="/en/teachers/">
              <div
                className="small-secondary text-gray2 hover:text-gray1"
                data-cy="hp-menu-teachers"
              >
                Find a teacher
              </div>
            </a>
            <a
              className="hidden p-2 mr-4 md:inline-block"
              href="/en/group-class/english"
            >
              <div
                className="small-secondary text-gray2 hover:text-gray1"
                data-cy="hp-menu-groupclass"
              >
                Group Class
              </div>
            </a>
            <a
              className="hidden p-2 mr-2 lg:inline-block"
              href="/en/community/for-you"
            >
              <div
                className="small-secondary text-gray2 hover:text-gray1"
                data-cy="hp-menu-community"
              >
                Community
              </div>
            </a>
            <a
              className="hidden p-2 mr-2 lg:inline-block"
              href="https://teach.italki.com/application"
            >
              <div
                className="small-secondary text-gray2 hover:text-gray1"
                data-cy="hp-menu-application"
              >
                Become a teacher
              </div>
            </a>
            <div
              className="hidden p-2 mr-2 lg:inline-block small-secondary text-gray2 hover:text-gray1 cursor-pointer"
              data-cy="hp-menu-signin"
            >
              Log in
            </div>
            <div
              className="hidden p-2 lg:inline-block small-secondary text-gray2 hover:text-gray1 cursor-pointer"
              data-cy="hp-menu-signup"
            >
              Sign up
            </div>
            <div className="lg:hidden">
              <div className="flex items-center lg:hidden">
                <div>
                  <div className="bm-overlay" style={overlayStyle}></div>
                  <div>
                    <div
                      className="bm-burger-button"
                      style={{ zIndex: '1000' }}
                    >
                      <div
                        className="bm-icon mobile-menu-icon"
                        id="react-burger-menu-btn"
                      >
                        <svg
                          height="24"
                          viewBox="0 0 24 24"
                          width="24"
                          xmlns="http://www.w3.org/2000/svg"
                          fill="#313140"
                        >
                          <g clip-rule="evenodd" fill-rule="evenodd">
                            <path d="M10.75 6a.75.75 0 01-.75.75H4a.75.75 0 010-1.5h6a.75.75 0 01.75.75zM14.75 18a.75.75 0 01-.75.75H4a.75.75 0 010-1.5h10a.75.75 0 01.75.75zM20.75 12a.75.75 0 01-.75.75H4a.75.75 0 010-1.5h16a.75.75 0 01.75.75z"></path>
                          </g>
                        </svg>
                      </div>
                    </div>
                  </div>
                  <div
                    id=""
                    className="bm-menu-wrap"
                    style={sidebarStyle}
                    aria-hidden="true"
                  >
                    <div className="bm-menu" style={contentStyle}>
                      <nav
                        className="bm-item-list"
                        style={{ height: '100%' }}
                      ></nav>
                      <div>
                        <div style={iconStyle}>
                          <span style={positionedStyle}>
                            <svg
                              fill="#313140"
                              height="24"
                              viewBox="0 0 24 24"
                              width="24"
                              xmlns="http://www.w3.org/2000/svg"
                              className="cursor-pointer"
                            >
                              <path
                                clip-rule="evenodd"
                                d="m5.5 4.4c-.3-.3-.8-.3-1.1 0s-.3.8 0 1.1l6.5 6.5-6.5 6.5c-.3.3-.3.8 0 1.1s.8.3 1.1 0l6.5-6.5 6.5 6.5c.3.3.8.3 1.1 0s.3-.8 0-1.1l-6.5-6.5 6.5-6.5c.3-.3.3-.8 0-1.1s-.8-.3-1.1 0l-6.5 6.5z"
                                fill-rule="evenodd"
                              ></path>
                            </svg>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </header>
    </>
  );
};

export default NavBar;
